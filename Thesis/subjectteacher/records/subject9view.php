  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <button type="button" class="btn btn-primary" onclick="window.open('sub9p1.php', '_blank')">1</button>
        <button type="button" class="btn btn-secondary" onclick="window.open('sub9p2.php', '_blank')">2</button>
        <br>
        <h3> Semester 2 - Quarter : </h3>
       
        <button type="button" class="btn btn-success" onclick="window.open('sub9p3.php', '_blank')">3</button>
        <button type="button" class="btn btn-danger" onclick="window.open('sub9p4.php', '_blank')">4</button>
      </div>
    </div>
  </div>
</div>
...
<div class="col-md-1 mb-3">
      <a class="link-primary me-2" href="subject9.php" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-class="tooltip" title="Add Grades">
...
<div class="mx-auto text-center text-wrap mb-3 text-white rounded-pill shadow" style="padding: 10px; background-image: linear-gradient(-20deg, #b721ff 0%, #21d4fd 100%);">
  <b style="white-space: nowrap;"><?php echo substr($_SESSION['sub9'], 0, 30); ?> </b>
</div>
...
$query = "SELECT b.id, b.studentname, b.subjectname, b.grade, b.teacher, b.section, b.adviser, b.gender, b.status,
    a.name AS user_name, a.sub9 AS user_sub9, a.sec1, a.sgh1, b.remarks, b.quarter, b.semester, b.sy,b.success
    FROM grade b, users a
    WHERE REPLACE(LOWER(b.subjectname), ' ', '') = REPLACE(LOWER('{$_SESSION['sub9']}'), ' ', '')  
    AND REPLACE(LOWER(b.teacher), ' ', '') = REPLACE(LOWER('$name'), ' ', '')
    AND REPLACE(LOWER(a.sub9), ' ', '') = REPLACE(LOWER('{$_SESSION['sub9']}'), ' ', '')
...
<td class="text-center" style="width:30%;"> <?php echo $rows["grade"]; ?></td>
  ...
  <a href="update9.php?id=<?php echo $rows['id'] ?>" data-bs-toggle="tooltip" data-bs-placement="top" title="Update Data">
    <b>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
       ...
       Grade in    <b> <?php echo $rows['subjectname']; ?> </b> ?
        ...
        <form class="delete" action="delete_grade9.php" method="POST">
...
<div>
          <script>
		function printPage() {
			window.open("printsubject9.php", "_blank");
		}
	</script>
<bR>
