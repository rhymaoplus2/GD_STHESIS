...
if (isset($_POST['submit'])) {
    ...
    foreach ($_POST['studentname'] as $index => $studentname) {
        ...
        if (mysqli_num_rows($query_run) > 0) {
            header("Location: ../subject9.php?error=Hello $name, You already submitted a grade for this student this quarter and semester. Go back and click update!");
            exit();
        } else {
            ...
            if (!$query_run) {
                header("Location: ../subject9.php?error=Error inserting grades into the database");
                exit();
            }
        }
    }
    header("Location: ../subject9view.php?success=Grades added successfully");
}
?>
