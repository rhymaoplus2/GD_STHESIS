...
if (isset($_POST['submit'])) {
    ...
    foreach ($_POST['studentname'] as $index => $studentname) {
        ...
        if (mysqli_num_rows($query_run) > 0) {
            ...
            $query = "INSERT INTO grade(studentname,subjectname,grade,teacher,section,adviser,firstname,middlename,lastname,gender,remarks,semester,quarter,sy,year,ts,studentid,session,date,time) 
                      VALUES ('$studentname','$subjectname','$grade','$teacher','$section','$adviser',
                      '$firstname','$middlename','$lastname','$gender','$remarks','$semester','$quarter','$sy','$year','$ts','$studentid','$session','$date','$time')";
            $query_run = mysqli_query($conn, $query);
