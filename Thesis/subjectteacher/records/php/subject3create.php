...
if(isset($_POST['submit']))
{
    ...
    foreach ($_POST['studentname'] as $index => $studentname)
    {
        ...
        $query = "SELECT * FROM grade WHERE studentname='$studentname' AND quarter='$quarter' AND subjectname='$subjectname'";
        ...
        if (mysqli_num_rows($query_run) > 0) {
            ...
            if (!$query_run)
            {
                header("Location: ../subject3.php?error=Error inserting grades into database");
                exit();
            }
        }
    }
