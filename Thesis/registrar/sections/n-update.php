...
if (isset($_GET['name'])) {
    ...
    // Loop through the subjectteacher columns
    for ($i = 1; $i <= 10; $i++) {
        $subjectteacher = validate($_POST['subjectteacher' . $i]);
        $sql = "UPDATE students SET subjectteacher{$i}='$subjectteacher' WHERE section='$name'";
        $result = mysqli_query($conn, $sql);
    }
