	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		if (!preg_match('/^\d{4}-\d{4}$/', $schoolyear)) {
			...
			if (mysqli_query($conn, $sql)) {
				echo "<p class='mt-3 alert alert-success'> <b> Academic year updating</b> 
<span class='wave-text'>.</span>
<span class='wave-text'>.</span>
<span class='wave-text'>.</span> <br> Updating Academic Year Would Take Some Time 


				<br> Just logout and login back , thanks : )</p>";
			} else {
				echo "<p class='mt-3 alert alert-danger'>Error updating school year: " . mysqli_error($conn) . "</p>";
			}
		}
	}
?>
