...
if ($num_subjects > 0) {
  $average_grade_4th = $total_grades_4th / $num_subjects;
} else {
  $average_grade_4th = 0;
}
...
arsort($total_grade_sum);

// Create a table to display the grades for each student and subject
echo "<table>";
echo "<thead>";
echo "<tr>";
echo "<th class='text-center'>MALE</th>";
...
foreach ($total_grade_sum as $studentname => $total_grade) {
    ...
    foreach ($unique_subjects as $subject) {
        if (isset($grades[$studentname][$subject]["4th"])) {
            echo "<td class='text-center'>" . $grades[$studentname][$subject]["4th"] . "</td>";
        } else {
...
if (ROUND($total_grade / $num)>= 90) {
  $honor = 'With honors';
} else if (ROUND($total_grade / $num) >= 95 && ROUND($total_grade / $num) <= 97) {
  $honor = 'With high honors';
} else if (ROUND($total_grade / $num) >= 98 && ROUND($total_grade / $num) <= 100) {
...
echo "<thead>";
echo "<tr>";
echo "<th class='text-center'>FEMALE</th>";
