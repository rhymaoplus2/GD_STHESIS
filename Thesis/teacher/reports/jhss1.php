...
FROM grade 
WHERE quarter IN ('FIRST', 'SECOND', 'THIRD', 'FOURTH') AND adviser = '$name'
...
// Create an array to store the quarter 1, 3, and 4 grades for each student
$quarter1_grades = array();
$quarter3_grades = array();
$quarter4_grades = array();
...
foreach ($quarter3_grades as $student_grades) {
    foreach ($student_grades as $grade) {
        $total_grades_3rd += $grade;
    }
}
foreach ($quarter4_grades as $student_grades) {
    foreach ($student_grades as $grade) {
        $total_grades_4th += $grade;
    }
}
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    if ($quarter == "FIRST") {
        ...
        // Add the grade to the quarter 3 grades for the student
        if (!isset($quarter3_grades[$studentname])) {
            $quarter3_grades[$studentname] = array();
        }
        $quarter3_grades[$studentname][$subjectname] = ($grade === null) ? 0 : $grade;
    } else if ($quarter == "FOURTH") {
        $grades[$studentname][$subjectname]["4th"] = ($grade === null) ? 0 : $grade;
        // Add the grade to the quarter 4 grades for the student
        if (!isset($quarter4_grades[$studentname])) {
            $quarter4_grades[$studentname] = array();
        }
        $quarter4_grades[$studentname][$subjectname] = ($grade === null) ? 0 : $grade;
    }
...
echo "<th class='text-center'>FIRST Sem<br>Final<br>Average</th><th></th><th></th>";
echo "</tr>
        <tr hidden class='text-center'>";
foreach ($unique_subjects as $subject) {
    echo "<th hidden  class='text-center'>1st<br>Quarter</th><th  hidden class='text-center'>2nd<br>Quarter</th><th  hidden class='text-center'>3rd<br>Quarter</th><th  hidden class='text-center'>4th<br>Quarter</th><th class='text-center'>FINAL</th>";
}
